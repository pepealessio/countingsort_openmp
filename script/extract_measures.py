...
RAW_FIELDS = ['size', 'range', 'n_th', 't_algo'] 
    ...
    for size_i, size in enumerate(ARRAY_RC):
        for range_i, rng in enumerate(ARRAY_RANGE):
            ...
            for nth_i, nth in enumerate(ARRAY_THS):
                ...
                m = np.genfromtxt(f"measures/raw/opt_{opt}_size_{size}_range_{rng}_threads_{nth}.csv", delimiter=',', invalid_raise=False)
            ...
            np.savetxt(f"measures/processed/{RAW_FIELDS[measure_on_index].upper()}_opt_{opt}_size_{size}_range_{rng}.csv", measures, delimiter=',', header=','.join([*RAW_FIELDS, "speedup", "efficiency"]))
...
for opt_i, opt in enumerate(ARRAY_OPT):
    for size_i, size in enumerate(ARRAY_RC):
        ...
        for range_i, rng in enumerate(ARRAY_RANGE):
            ...
            measures = np.genfromtxt(f"measures/processed/{RAW_FIELDS[measure_on_index].upper()}_opt_{opt}_size_{size}_range_{rng}.csv", delimiter=',', invalid_raise=False)
            
            ax1.plot(t, measures[:, -2][1:], '^-', label=f"r. {rng}")
            ax2.plot(t, measures[:, -1][1:], '.--')

        fig.suptitle(f'{RAW_FIELDS[measure_on_index].upper()} ({TITLE}) -  Size:{size} Opt:O{opt}', fontsize=15)
        ax1.set_xlabel("#CPU")
        ...
        plt.savefig(f"measures/plots/{RAW_FIELDS[measure_on_index].upper()}_opt_{opt}_size_{size}.png")
        plt.close()
